## Overview
This project is a company search application implemented using Spring Boot 3.2.5 and Java 19

The application exposes an endpoint that uses the `TruProxyAPI` to do a company and officer lookup
...
* where false flag request parameter is specified all the companies are returned
* The officers of each company is included in the company details`officers`
* Only included officers that are active (`resigned_on` is not present in that case)
* Paging is ignored
* Added tests at controller and service level, more unit and integration test scenarios could be added.
...
## Bonus - I would have loved to do the bonus task, but could not due to time constraint.
